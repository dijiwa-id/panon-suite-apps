...
const DevelopContext = createContext<DevelopContextType | undefined>(undefined);
...
export const DevelopProvider = ({ children }: { children: ReactNode }) => {
  ...
  const savePipeline = (pipeline: Omit<Pipeline, 'id' | 'updatedAt'>) => {
    const existing = pipelines.find(p => p.name === pipeline.name);
    const now = new Date().toISOString();
    
    if (existing) {
      toast.success("Pipeline updated successfully");
      setPipelines(prev => prev.map(p => p.id === existing.id ? { ...p, ...pipeline, updatedAt: now } : p));
    } else {
      const newId = `p-${Date.now()}`;
      toast.success("Pipeline created successfully");
      setPipelines(prev => [...prev, { ...pipeline, id: newId, updatedAt: now }]);
    }
  };

  const addApplication = (app: Omit<Application, 'id' | 'status' | 'uptime' | 'endpoint' | 'nodesCount'>) => {
    // Find pipeline to get node count
    const pipeline = pipelines.find(p => p.id === app.pipelineId);
    
    const newApp: Application = {
      ...app,
      id: `APP-0${applications.length + 1}`,
      status: 'stopped',
      uptime: '-',
  ...
  return (
    <DevelopContext.Provider value={{ blocks, addBlock, pipelines, savePipeline, applications, addApplication, toggleApplicationStatus, activePipelineId, setActivePipelineId }}>
      {children}
...
export const useDevelop = () => {
  const context = useContext(DevelopContext);
  if (context === undefined) {
    throw new Error('useDevelop must be used within a DevelopProvider');
  }
  return context;
};
