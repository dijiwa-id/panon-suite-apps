...
export const TrainProvider = ({ children }: { children: ReactNode }) => {
  ...
  const [trainingJobs, setTrainingJobs] = useState<TrainingJob[]>(defaultJobs);
  const [models, setModels] = useState<Model[]>(defaultModels);

  const addDataset = (ds: Omit<Dataset, 'id' | 'samples' | 'annotations' | 'size' | 'lastUpdated' | 'status'>) => {
  ...
  const updateDatasetStatus = (id: string, status: Dataset['status']) => {
    setDatasets(prev => prev.map(ds => ds.id === id ? { ...ds, status } : ds));
  };

  const incrementDatasetSamples = (id: string, numFrames: number) => {
  ...
  const incrementDatasetAnnotations = (id: string, count: number) => {
     setDatasets(prev => prev.map(ds => {
      if (ds.id === id) {
        return {
          ...ds,
  ...
  const addTrainingJob = (job: Omit<TrainingJob, 'id' | 'status' | 'timeRemaining' | 'epoch' | 'map'>) => {
    const newJob: TrainingJob = {
      ...job,
      id: `TR-09${trainingJobs.length + 3}`,
      status: 'training',
    ...
    setModels(prev => [{
        id: `MOD-2026-00${models.length + 1}`,
        name: newJob.name,
        version: 'v1.0.0',
  ...
  return (
    <TrainContext.Provider value={{ datasets, addDataset, updateDatasetStatus, incrementDatasetSamples, incrementDatasetAnnotations, trainingJobs, addTrainingJob, models }}>
      {children}
