...
export const UserSettings = () => {
  ...
  const [name, setName] = useState(user?.name || 'M Iqbal');
  const [email, setEmail] = useState(user?.email || 'iqbal@panon.com');
  const [bio, setBio] = useState('');
  
  const [notifications, setNotifications] = useState([
    { id: 1, title: "Critical System Alerts", description: "Get notified when a camera goes offline or a critical error occurs.", active: true },
    { id: 2, title: "Violation Detections", description: "Receive immediate updates on PPE or safety violations.", active: true },
    { id: 3, title: "Weekly Reports", description: "A summary of the previous week's performance metrics.", active: false },
    { id: 4, title: "New Features", description: "Updates on Panon Suite platform features and changes.", active: false }
  ]);
      ...
      <div className="max-w-5xl mx-auto flex flex-col gap-6">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
          ...
          <div className="flex items-center gap-3 shrink-0 mb-3">
             <Button variant="outline" onClick={handleLogout} className="gap-2 text-red-500 hover:text-red-500 hover:bg-red-500/10 border-red-200 dark:border-red-500/20">
               <LogOut size={14} />
               <span>Log Out</span>
             </Button>
                     ...
                     <div className="flex flex-col md:flex-row items-start md:items-center gap-6">
                       ...
                       <div className="flex-1">
                         ...
                         <div className="flex flex-wrap items-center gap-3 mt-2">
                           <span className="px-2 py-1 bg-[#52C5F3]/10 border border-[#52C5F3]/20 text-[#52C5F3] text-[10px] font-bold tracking-wide rounded">{user?.role || 'System Administrator'}</span>
                           <span className="px-2 py-1 bg-[#1c1c1c] border border-gray-700 text-white text-[10px] font-bold tracking-wide rounded">{user?.organization || 'Panon'} Organization (Tenant)</span>
                ...
                <Card className="p-0 overflow-hidden animate-in fade-in slide-in-from-bottom-2 duration-300">
                  ...
                  <div className="p-6 space-y-6">
                    <div className="flex items-center justify-between p-4 bg-gray-50 dark:bg-[#161616] border border-gray-200 dark:border-[#222] rounded-xl">
                      ...
                      <button onClick={() => toast.success('2FA Setup email sent.')} className="text-[11px] font-black tracking-widest uppercase text-[#52C5F3] hover:text-[#3baee0] transition-colors">Enable</button>
                    </div>
                ...
                <Card className="p-0 overflow-hidden animate-in fade-in slide-in-from-bottom-2 duration-300">
                  ...
                  <div className="p-6 space-y-4">
                     {notifications.map((notif) => (
                        <div key={notif.id} className="flex items-start justify-between p-4 bg-gray-50 dark:bg-[#161616] border border-gray-200 dark:border-[#222] rounded-xl">
                          <div>
                          ...
                          <button onClick={() => toggleNotification(notif.id)} className={cn("relative inline-flex h-5 w-9 shrink-0 cursor-pointer items-center justify-center rounded-full focus:outline-none focus:ring-2 focus:ring-[#52C5F3] focus:ring-offset-2 transition-colors", notif.active ? "bg-[#52C5F3]" : "bg-gray-300 dark:bg-[#333]")}>
