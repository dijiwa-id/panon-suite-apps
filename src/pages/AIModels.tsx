  ...
  return (
    <main className="flex-1 overflow-y-auto bg-[#161616] p-6 lg:p-8 text-gray-200 transition-colors">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
        <div>
          <h1 className="text-xl font-bold tracking-tight text-white mb-1">AI Models Workspace</h1>
          <p className="text-gray-400 text-xs font-medium">Review, evaluate, and export trained model artifacts.</p>
        </div>
      </div>
        ...
        <div className="p-5 border-b border-[#2a2a2a] flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-[#1a1a1a]">
          ...
          <div className="flex w-full sm:w-auto">
             <div className="bg-[#151515] px-4 py-2 rounded-xl border border-[#2a2a2a] flex items-center gap-2 flex-1 sm:flex-none focus-within:border-accent/50 focus-within:ring-1 focus-within:ring-accent/50 transition-all">
                ...
                <input type="text" placeholder="Search models by name or tag..." className="bg-transparent outline-none text-xs font-medium text-gray-200 w-full sm:w-64 placeholder-gray-600" />
             </div>
          </div>
        </div>
                ...
                <tr key={model.id} className="hover:bg-white/5 transition-colors group">
                  <td className="px-5 py-4">
                      <div className="flex items-center gap-2 mb-0.5">
                          ...
                          <span className="bg-[#2a2a2a] text-gray-300 font-mono px-1.5 py-0.5 rounded text-[10px]">{model.version}</span>
                      </div>
                      <div className="text-[10px] text-gray-500 font-mono tracking-wide">{model.id}</div>
                  </td>
                  <td className="px-5 py-4">
                      <div className="text-gray-300 font-medium mb-0.5">{model.architecture}</div>
                      <div className="text-[10px] text-gray-500">{model.param} params • {model.size}</div>
                  </td>
                  <td className="px-5 py-4">
                  ...
                  <td className="px-5 py-4">
                      <div className="flex flex-wrap gap-1.5">
                          {model.tags.map(t => (
                              <span key={t} className="bg-accent/10 border border-accent/20 text-accent px-1.5 py-0.5 rounded text-[10px] font-bold tracking-wide">{t}</span>
                          ))}
                      </div>
                  </td>
                  <td className="px-5 py-4">
                    <div className="flex items-center gap-1.5">
                         {model.status === 'ready' && <CheckCircle2 size={14} className="text-gray-400" />}
                         ...
                         <span className="text-[11px] font-medium text-gray-300 capitalize">{model.status}</span>
                    </div>
                  </td>
