...
const AddWorkstationModal = ({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
      <div className="bg-[#1e1e1e] border border-[#2a2a2a] rounded-2xl shadow-2xl w-full max-w-2xl overflow-hidden flex flex-col">
        ...
        <div className="flex items-center justify-between p-6 border-b border-[#2a2a2a] bg-[#1a1a1a]">
            <div>
                ...
                <p className="text-gray-400 text-xs font-medium mt-1">Add a new edge processing node to the cluster.</p>
            </div>
        ...
        <div className="p-6 overflow-y-auto max-h-[70vh]">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                <div className="space-y-5">
                    <div>
                        <label className="block text-xs font-semibold text-gray-400 mb-2 flex items-center gap-1.5">Node ID / Identifier</label>
                        <input type="text" placeholder="e.g., WS-005 / ALPR Node" className="w-full bg-[#161616] border border-[#2a2a2a] rounded-xl px-4 py-2.5 text-sm text-gray-200 outline-none focus:border-accent/50 focus:ring-1 focus:ring-accent/50 transition-all" />
                    </div>
                    <div>
                        <label className="block text-xs font-semibold text-gray-400 mb-2 flex items-center gap-1.5"><Network size={14}/> IP Address</label>
                        <input type="text" placeholder="192.168.1.x" className="w-full bg-[#161616] border border-[#2a2a2a] rounded-xl px-4 py-2.5 text-sm text-gray-200 outline-none focus:border-accent/50 focus:ring-1 focus:ring-accent/50 transition-all font-mono" />
                    </div>
                </div>

                <div className="space-y-5">
                    <div>
                        <label className="block text-xs font-semibold text-gray-400 mb-2 flex items-center gap-1.5"><Info size={14}/> Description / Purpose</label>
                        <input type="text" placeholder="Enter purpose..." className="w-full bg-[#161616] border border-[#2a2a2a] rounded-xl px-4 py-2.5 text-sm text-gray-200 outline-none focus:border-accent/50 focus:ring-1 focus:ring-accent/50 transition-all" />
                    </div>
                    <div>
                        <label className="block text-xs font-semibold text-gray-400 mb-2 flex items-center gap-1.5"><Shield size={14}/> Access Credential (Token/Key)</label>
                        <input type="password" placeholder="••••••••••••" className="w-full bg-[#161616] border border-[#2a2a2a] rounded-xl px-4 py-2.5 text-sm text-gray-200 outline-none focus:border-accent/50 focus:ring-1 focus:ring-accent/50 transition-all font-mono" />
                    </div>
                </div>
            </div>
            ...
            <div className="bg-[#1a1a1a] p-4 rounded-xl border border-[#2a2a2a]">
                <h3 className="text-sm font-bold text-white flex items-center gap-2 mb-4">
                    <Cpu size={16} className="text-gray-500" /> Auto-Discovered Hardware
                </h3>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                     <div className="bg-[#151515] p-3 rounded-lg border border-[#2a2a2a]">
                         <div className="text-[10px] text-gray-500 font-bold uppercase tracking-wider mb-1">Architecture</div>
                         <div className="text-xs font-medium text-gray-300">x86_64</div>
                     </div>
                     <div className="bg-[#151515] p-3 rounded-lg border border-[#2a2a2a]">
                         <div className="text-[10px] text-gray-500 font-bold uppercase tracking-wider mb-1">Processor</div>
                         <div className="text-xs font-medium text-gray-300">Pending Scan</div>
                     </div>
                     <div className="bg-[#151515] p-3 rounded-lg border border-[#2a2a2a]">
                         <div className="text-[10px] text-gray-500 font-bold uppercase tracking-wider mb-1">GPU Compute</div>
                         <div className="text-xs font-medium text-gray-300">Pending Scan</div>
                     </div>
                     <div className="bg-[#151515] p-3 rounded-lg border border-[#2a2a2a]">
                         <div className="text-[10px] text-gray-500 font-bold uppercase tracking-wider mb-1">NPU Engine</div>
                         <div className="text-xs font-medium text-gray-300">Pending Scan</div>
                     </div>
                </div>
            </div>
  ...
  return (
    <main className="flex-1 overflow-y-auto bg-[#161616] p-6 lg:p-8 text-gray-200 transition-colors">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
        <div>
          <h1 className="text-xl font-bold tracking-tight text-white mb-1">Workstation Management</h1>
          <p className="text-gray-400 text-xs font-medium">Manage and monitor processing edge nodes.</p>
        </div>
        ...
        <div className="p-5 border-b border-[#2a2a2a] flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-[#1a1a1a]">
          ...
          <div className="flex gap-3 w-full sm:w-auto">
             <div className="bg-[#151515] px-4 py-2 rounded-xl border border-[#2a2a2a] flex items-center gap-2 flex-1 sm:flex-none focus-within:border-accent/50 focus-within:ring-1 focus-within:ring-accent/50 transition-all">
                ...
                <input type="text" placeholder="Search by ID, IP..." className="bg-transparent outline-none text-xs font-medium text-gray-200 w-full sm:w-48 placeholder-gray-600" />
             </div>
          </div>
        </div>
              ...
              {workstations.map((ws) => (
                <tr key={ws.id} className="hover:bg-white/5 transition-colors group">
                  <td className="px-5 py-4">
                    <div className={cn("inline-flex items-center gap-1.5 px-2 py-1 rounded-md border text-[10px] font-bold uppercase tracking-wider", ws.status === 'Running' ? "bg-green-500/10 border-green-500/20 text-green-400" : "bg-red-500/10 border-red-500/20 text-red-400")}>
                        <span className={cn("w-1.5 h-1.5 rounded-full", ws.status === 'Running' ? "bg-green-500" : "bg-red-500")} />
                        {ws.status}
                    </div>
                  </td>
                  <td className="px-5 py-4">
                    <div className="font-semibold text-white text-sm mb-0.5">{ws.id.split(' / ')[1]}</div>
                    <div className="text-[11px] text-gray-500 font-mono mb-1">{ws.id.split(' / ')[0]} • {ws.ip}</div>
                    <div className="text-[10px] text-gray-400">{ws.description}</div>
                  </td>
                  <td className="px-5 py-4">
                    <div className="text-gray-300 font-medium">{ws.spec}</div>
                  </td>
                  <td className="px-5 py-4">
                  ...
                  <td className="px-5 py-4 text-right">
                    <div className="font-mono text-xs text-gray-300">{ws.load}</div>
                  </td>
                </tr>
              ))}
            </tbody>
