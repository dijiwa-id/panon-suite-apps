...
- **Body Background (Dark):** `#0e1016`
- **Card Background (Dark):** `#1e1e1e`
- **Text (Dark):** Neutral gray-scale, consistent with global theme.

## 2. Component Design
### Cards
- **Structure:** Rounded corners (`rounded-xl`), border-gray-100 (light) / border-[#1f232d] (dark).
- **Shadows:** Subtle `shadow-sm` for depth.
- **Backgrounds:** Use `#1e1e1e` for consistency in dark mode.

### Tables
- **Headers:** Minimalist header with `text-[10px]`, `font-black`, `uppercase`, `tracking-widest`.
...
- **Classes:** `bg-[#1c1c1c] border border-gray-700 h-[29px] text-white rounded-full text-xs font-bold uppercase tracking-wide w-24 leading-[12px] pl-5 hover:bg-[#2a2a2a] transition-colors`.
- **Usage:** Action buttons (e.g., "New", "Submit" in certain contexts).

### Typography
- **Headings:** Bold/Black weights with `tracking-tight`.
- **UI Labels:** Use small font sizes (`text-sm`, `text-[10px]`) and `uppercase` for secondary labels (e.g., table headers, card headers).
